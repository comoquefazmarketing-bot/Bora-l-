/* @author Felipe Makarios | Lead Architect 
  PORTAL MANDA LÁ - UX/UI PREMIUM DARK
*/
import React, { useState, useEffect } from "react";
import { 
  Users, Utensils, Beer, Droplets, Flame, 
  Package, MessageSquare, ChevronDown, Zap, ChevronRight 
} from "lucide-react";

export default function ChurrascoCalc() {
  ...
  // Escuta o disparo do Header (CustomEvent)
  useEffect(() => {
    const handleOpen = () => {
      setIsOpen(true);
  ...
  const res = {
    ...
    gelo: Math.ceil((counts.homens + counts.mulheres + counts.criancas) / 8),
    descartaveis: Math.ceil((counts.homens + counts.mulheres + counts.criancas) * 1.5)
  };
      ...
      <div className="bg-[#0A0A0A] rounded-b-[70px] border-x-[10px] border-b-[10px] border-white p-8 md:p-14 pt-24 relative shadow-[0_60px_150px_rgba(0,0,0,1)]">
        ...
        <div className="flex flex-col lg:flex-row gap-12">
          ...
          <div className="flex-1 space-y-5">
            ...
            {['homens', 'mulheres', 'criancas'].map(type => (
              <div key={type} className="bg-white/5 p-6 rounded-[35px] border border-white/5 flex justify-between items-center group hover:bg-white/10 transition-all">
                 ...
                 <div className="flex items-center gap-5">
                    <button onClick={() => updateCount(type, -1)} className="w-10 h-10 rounded-xl bg-white/5 text-white font-black hover:bg-red-500 transition-all">-</button>
                    <span className="text-white font-black text-3xl italic min-w-[30px] text-center">{counts[type]}</span>
                    <button onClick={() => updateCount(type, 1)} className="w-10 h-10 rounded-xl bg-[#00BFA6] text-black font-black hover:scale-110 transition-all">+</button>
                 </div>
              </div>
            ))}
          </div>
          ...
          <div className="flex-[1.5] space-y-10">
            <div className="bg-[#00BFA6] rounded-[50px] p-8 text-black grid grid-cols-2 md:grid-cols-3 gap-8 shadow-2xl relative overflow-hidden">
              ...
              <Item val={res.carvao} unit="SC" label="Carvão" icon={<Flame size={16}/>} />
              <Item val={res.gelo} unit="SC" label="Gelo" icon={<Zap size={16}/>} />
              <Item val={res.descartaveis} unit="IT" label="Kits" icon={<Package size={16}/>} />
            </div>

            <div className="space-y-4">
              <h4 className="text-white/30 font-black uppercase text-[9px] tracking-[0.4em] px-4 italic">2. Gerar pedido via WhatsApp:</h4>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {suppliers.map((s, i) => (
                  <button key={i} onClick={() => sendWhats(s)} className="flex items-center justify-between bg-white/5 border border-white/10 p-6 rounded-[40px] hover:bg-[#25D366] group transition-all active:scale-95">
        ...
        <button onClick={() => setIsOpen(!isOpen)} className="absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-full bg-white px-20 py-5 rounded-b-[45px] shadow-2xl flex items-center gap-5 border-x-[10px] border-b-[10px] border-white group transition-all">
          <span className="font-black text-[11px] uppercase tracking-[0.5em] text-black">
