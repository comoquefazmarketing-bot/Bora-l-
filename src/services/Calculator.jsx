...
import { Calculator as CalcIcon, X, ArrowRight, Wallet, Flame } from "lucide-react";
  ...
  return (
    <div className={`fixed top-0 left-1/2 -translate-x-1/2 z-[999] w-full max-w-5xl transition-all duration-700 ${isOpen ? 'translate-y-0' : '-translate-y-[calc(100%-48px)]'}`}>
      <div className="bg-[#1A1A1A] rounded-b-[70px] border-x-[8px] border-b-[8px] border-white p-10 pt-16 relative text-white shadow-2xl">
             ...
             {Object.keys(counts).map(k => (
               <div key={k} className="flex justify-between items-center bg-white/5 p-4 rounded-3xl border border-white/5">
                 ...
                 <div className="flex gap-2 items-center">
                   <button onClick={() => handleInputChange(k, counts[k] - 1)} className="w-10 h-10 bg-white/10 rounded-xl font-black text-xl">-</button>
                   <input 
                    type="number" 
                    value={counts[k]} 
                    onChange={(e) => handleInputChange(k, e.target.value)}
                    className="w-16 bg-transparent border-b-2 border-[#00BFA6] text-center font-black italic text-2xl text-[#00BFA6] focus:outline-none [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                   />
                   <button onClick={() => handleInputChange(k, (Number(counts[k]) || 0) + 1)} className="w-10 h-10 bg-white/10 rounded-xl font-black text-xl">+</button>
                 </div>
               </div>
             ))}
          </div>
            ...
            <div className="p-6 bg-[#00BFA6]/10 border border-[#00BFA6]/20 rounded-[40px] flex gap-5 items-center">
               ...
               <div>
                 ...
                 <p className="text-[12px] leading-snug text-white/90 font-bold uppercase italic">
                   Use esse cálculo e gere um orçamento automaticamente nos nossos parceiros, assim você já faz a divisão com a galera.
                 </p>
               </div>
            </div>
          </div>
        ...
        <button onClick={() => setIsOpen(!isOpen)} className="absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-full bg-white px-16 py-5 rounded-b-[45px] font-black text-xs uppercase text-black shadow-2xl border-x-[8px] border-b-[8px] border-white hover:bg-[#00BFA6] transition-all">
